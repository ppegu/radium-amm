...
class SolanaTrader {
  ...
  private solPk = process.env.SOLANA_PRIVATE_KEY;
  private radiumPI = process.env.RADIUM_DEX_PROGRAM_ID;
  ...
  constructor() {
    ...
    this.solanaAccount = Keypair.fromSecretKey(
      new Uint8Array(JSON.parse(this.solPk || ""))
    );
    this.radiumProgramId = new PublicKey(this.radiumPI || "");
  }
  ...
  public async buy(
    ...
    const buyTransaction = await this.createTransaction(
      token,
      amount,
      quantity,
      numOrders,
    ...
    const signature = await this.connection.sendTransaction(buyTransaction, [
      this.solanaAccount,
    ]);
  ...
  public async sell(
    ...
    const sellTransaction = await this.createTransaction(
      token,
      amount,
      quantity,
      numOrders,
    ...
    const signature = await this.connection.sendTransaction(sellTransaction, [
      this.solanaAccount,
    ]);
...
(async () => {
  ...
  const tokenToTrade = new PublicKey(String(process.env.TOKEN_PUBLIC_KEY));
